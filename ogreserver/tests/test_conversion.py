...
def test_search(datastore, user, rethinkdb, s3bucket, conversion):
    ...
    expected_params = [
        ((conversion.flask_app, ebook_id, version_id, datastore.generate_filename(file_hash), 'egg'),),
        ((conversion.flask_app, ebook_id, version_id, datastore.generate_filename(file_hash), 'mobi'),),
    ]
